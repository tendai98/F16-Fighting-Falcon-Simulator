...
function updateMissiles(dt){
  ...
  for (let i=world.sams.length-1;i>=0;i--){
    ...
    const groundExact = (m.groundPos && trueTgtPos) ? {...trueTgtPos} : null;
    let guidanceTgt = tgtPos;
    let terminalGround = false;
    if (m.groundPos && groundExact){
      const hDist = Math.hypot(groundExact.x-m.pos.x, groundExact.y-m.pos.y);
      terminalGround = hDist < ((m.kind==='HARM' || /HARM/.test(m.kind||'')) ? 1300 : 950);
      const loft = terminalGround ? 22 : clamp(hDist*0.08, 140, 720);
      // Shallow mid-course loft clears visible terrain, but terminal guidance
      // returns to the exact stored designation point so TGP impacts look honest.
      guidanceTgt = {x:groundExact.x, y:groundExact.y, z:groundExact.z + loft};
    }
    const maskCheckTgt = m.groundPos ? guidanceTgt : tgtPos;
    const maskMargin = m.groundPos ? 18 : 35;
    const maskLimit = m.groundPos ? (terminalGround ? 0.70 : 0.92) : 0.985;
    const block = missileTerrainBlock(m, maskCheckTgt, maskMargin);
    if (block && block.u < maskLimit){
      terrainBlocked = true;
      terrainImpact = block;
      m._maskT = (m._maskT||0) + dt;
      m.energy -= (m.groundPos ? 0.18 : (isRadarMissile(m)?0.26:0.18)) * dt;
      if (m.groundPos){
        // Terrain still masks real shots, but do not treat harmless grazing near
        // the designated terminal point as a full miss.
      ...
      if (m.groundPos){ m._terrainMaskedTarget = false; tgtPos = terminalGround ? groundExact : guidanceTgt; }
    }
    ...
    if (m.groundPos){                          // guided surface attack (AGM / HARM)
      const exact = trueTgtPos ? {...trueTgtPos} : tgtPos;
      const gd = vlen(vsub(tgtPos, m.pos));
      const exactD = vlen(vsub(exact, m.pos));
      const tightHit = exactD < ((m.kind==='HARM' || /HARM/.test(m.kind||'')) ? 34 : 28);
      const tightPass = (m._gpdExact!==undefined && m._gpdExact < 46 && exactD > m._gpdExact && exactD < 70);
      const terminalGroundHit = hitGround && exactD < 44;
      const directImpact = tightHit || tightPass || terminalGroundHit;
      if (directImpact || hitGround || missileRangeExceeded(m, prof) || m.energy<=0.03){
        const ip = directImpact ? exact : {x:m.pos.x, y:m.pos.y, z:Math.max(m.pos.z, terrainH(m.pos.x,m.pos.y)+1)};
        addEffect(ip, directImpact?1.35:0.95);
        if (window.recordMissionEvent) recordMissionEvent('projectile_impact', { weapon:m.weapon||m.kind||'MISSILE', kind:m.kind||'missile', x:ip.x, y:ip.y, z:ip.z, terrainMasked:!!m._terrainMaskedTarget, directImpact:!!directImpact });
        if (m._terrainMaskedTarget){
        ...
        } else if (!directImpact){
          banner(hitGround?'MISSILE TERRAIN HIT':'MISSILE MISS', 1.0);
          ...
          if (m.kind==='HARM' || /HARM/.test(m.kind||'')){
            ...
            bombImpact(exact.x, exact.y, m.weapon||m.kind||'AGM');
          }
        }
        world.sams.splice(i,1);
      } else { m._gpd = gd; m._gpdExact = exactD; }
      continue;
    }
...
function updateThreats(dt){
  ...
  const aggr = world._aggr || 0.8;
  const fireGap = 4.2 / aggr;                                      // quick re-shot spacing once detected
  const reactionDelay = Math.max(0.35, 0.75 / aggr);               // fast first shot inside ring
  for (const th of world.threats){
    ...
    if (inRing){
      anyTrack=true;
      if (!th._wasInRing) th._dwell = 0;
      th._wasInRing = true;
      th._dwell = (th._dwell||0) + dt;
      if (th._dwell > reactionDelay && world.t - (th.launchT||-999) > fireGap && redInAir < RED_CAP && (!world._tutorial || world._tutFire)){
    ...
    } else { th._dwell = 0; th._wasInRing = false; }
  }
